...
import { TestimonialCard, TechItem } from "../_components/service-primitives";
...
// Custom components for additional sections (matching design system)
import {
  ShoppingCart,
  Smartphone,
  Package,
...
export function PosSystemsContent() {
  ...
  return (
    <>
      ...
      {/* POS Highlights (original section) */}
      <ServiceSection className="py-16">
        <SectionHeading
          eyebrow="POS Route"
      ...
      {/* Operational Payoff & Rollout Path (original) */}
      <ServiceSection className="py-8">
      ...
      {/* UI Showcase - POS Dashboard & Interface */}
      <ServiceSection className="py-8">
        <div className="text-center max-w-2xl mx-auto mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">
          ...
          {[
            {
              ...
              desc: "Average transaction time reduced",
            },
            {
              icon: Clock,
              title: "99.95% Uptime",
              desc: "Offline mode ensures no lost sales",
            },
            {
              icon: Award,
              title: "Inventory Accuracy",
              desc: "Real‑time sync across all locations",
            },
            {
              icon: Users,
              title: "Staff Friendly",
              desc: "Minimal training, intuitive UI",
            },
          ].map((item) => (
            <div
              key={item.title}
              className="bg-slate-50 rounded-xl p-6 text-center hover:shadow-lg transition-all"
            >
      ...
      {/* Final Call‑to‑Action - Fixed SVG pattern removed */}
      <ServiceSection className="py-12 pb-24">
        <div className="bg-gradient-to-r from-blue-700 to-blue-900 rounded-3xl p-12 text-center relative overflow-hidden">
          {/* Replaced problematic SVG pattern with a safe dot pattern */}
      ...
      {/* Branch Setup Decisions & Related Links (original) */}      <ServiceSection className="py-12 pb-12">
        <BookDemoCTA />
      </ServiceSection>
